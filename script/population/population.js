    ...
    new Vue({
        ...
        methods: {
            ...
            EditData: function(params) {
                ...
                this.houseNo = params.hjh; //户籍号
                this.weichat = params.weiChat; //微信
                if (params.sex == "男") {
                    params.sex = "1";
                } else {
                    params.sex = "2";
                ...
                var order = parseInt(that.politicsId)
                if (order >= 3 && order <= 11) {
                    that.notparty = false;
                } else {
                    that.notparty = true;;
                }
                this.partytime = params.partyTime; //入党时间
                this.partyplace = params.partySpace; //入党地点
                ...
                console.log("sdasda+    " + this.residentId);
            },
            sexf: function() { //性别
                console.log("性别");
                var that = this;
                if (!this.isBrowser && !this.isdisabled) {
            ...
            checkIdNumber: function() {
                var info = checkId(this.identity);
                var idNumbers = parseInt(info)
                var json = {};
                json.idNumber = this.identity
                var that = this;
                var result = "";
                result = JSON.stringify(json);
                if (idNumbers) {
                  if(!this.idDisabled){
                    UICore.showLoading("正在查询...", "请稍候");
                    this.isBrowser = true;
                    console.log(UICore.serviceUrl + 'mobile/mobileInterfaceForPopulation.shtml?act=getPopulationCoreInfoByIDCard&data=' + result);
                    api.ajax({
                        url: UICore.serviceUrl + 'mobile/mobileInterfaceForPopulation.shtml?act=getPopulationCoreInfoByIDCard&data=' + result,
                        method: 'get',
                    }, function(ret, err) {
                        api.hideProgress();
                        that.isBrowser = false;
                        if (ret) {
                            console.log(JSON.stringify(ret));
                            if (ret.success) {
                                that.isdisabled = true;
                                that.name = ret.data[0].name; //姓名
                                that.sexId = ret.data[0].sex;
                                that.sex_arr.forEach(function(value) {
                                    if (value.key == that.sexId)
                                        that.sex = value.text;
                                }); //性别
                                that.birthday = ret.data[0].birthday.substring(0, 10); //出生日期
                                that.nationId = ret.data[0].nation;
                                that.nation_arr.forEach(function(value) {
                                    if (value.key == that.nationId)
                                        that.nation = value.text;
                                }); //民族
                                that.politicsId = ret.data[0].politicalStatus;
                                that.politicalStatus_arr.forEach(function(value) {
                                    if (value.key == that.politicsId)
                                        that.politics = value.text;
                                }); //政治面貌
                                that.partyplace = ret.data[0].partySpace; //入党地点
                                that.partytime = ret.data[0].partyTime; //入党时间

                                that.religionId = ret.data[0].religion;
                                that.religion_arr.forEach(function(value) {
                                    if (value.key == that.religionId)
                                        that.religion = value.text;
                                }); //宗教信仰

                                that.degreeId = ret.data[0].degree;
                                that.degree_arr.forEach(function(value) {
                                    if (value.key == that.degreeId)
                                        that.degree = value.text;
                                }); //文化程度

                                that.registTypeId = ret.data[0].accountProperties; //户口性质
                                that.accountProperties_arr.forEach(function(value) {
                                    if (value.key == that.registTypeId)
                                        that.registType = value.text;
                                }); //文化程度

                                that.socialOfficeId = ret.data[0].position; //社会任职
                                that.position_arr.forEach(function(value) {
                                    if (value.key == that.socialOfficeId)
                                        that.socialOffice = value.text;
                                }); //文化程度

                                that.registerAddress = ret.data[0].regAddress; //户籍详址
                            } else {
                                that.province = get_province_from_idcard(that.identity); //户籍省
                                that.city = get_only_city_from_idcard(that.identity); //户籍市
                                that.county = get_only_county_from_idcard(that.identity); //户籍区/县
                                that.sex = get_sex_from_idcard(that.identity);
                                that.sex_arr.forEach(function(value, index, arr) {
                                    if (value.text == that.sex) {
                                        that.sexId = value.key
                                    }
                                })
                                that.birthday = get_Birthday_from_idcard(that.identity);
                            }
                        } else {
                            alert(JSON.stringify(err));
                        }
            ...
            partytimef: function() {
                var that = this;
                if (!this.isBrowser && !this.isdisabled) {
                    UICore.openTimeComponent2(this.partytime);
                    api.addEventListener({
                        name: 'buildingTime'
                    }, function(ret, err) {
                        if (ret) {
                            that.partytime = ret.value.key1;
                        } else {
                            alert(JSON.stringify(err));
                        }
                    });
            ...
            submit: function() {
                ...
                if (this.isBrowser) {
                    ...
                    if (!this.isNew) { //如果是编辑
                        ...
                        postjson.residentId = this.residentId;
                        console.log("aaaa");
                    }
                    ...
                    // if (this.nation != "请选择" && this.nationId) //民族
                    ...
                    if (this.partytime) {
                        postjson.partyTime = this.partytime; //入党时间
                    }
                    if (this.partyplace) {
                        postjson.partySpace = this.partyplace; //入党地点
                    }
                    ...
                    postjson.weiChat = this.weichat; //微信
