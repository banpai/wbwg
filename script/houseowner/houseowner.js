    ...
    new Vue({
        ...
        created: function() {
            ...
            jsonData.data.forEach(function(value) {
                ...
                if (value.parentKey == "ResidentProperty") { //户属性
                    that.residentProperty_arr.push({
                        text: value.extendAttributeValue,
                        status: 'normal',
                        key: value.extendAttributeKey
                    });
            ...
            if (that.isBrowser) { //在浏览状态下 显示数据
                ...
                api.ajax({
                    ...
                    if (ret.success) {
                        ...
                        that.register = houseinfo.rhdjsj; //入户登记时间
                        ...
                        var index = parseInt(houseownerinfo.residentProperty);
                        if (!isNaN(index)) {
                            that.residentProperty_arr[index].status = "selected";
                            that.residentProperty = that.residentProperty_arr[index].text;
                        }; //户属性
                        ...
                        that.huxing = houseinfo.huXing; //户型
                        that.carNum = houseinfo.carNum; //车辆数
                        that.carId = houseinfo.carID;//车牌号
                        // that.houseownerId = houseinfo.idNumber; //屋主身份证
                        // that.houseownerName = houseinfo.homeOwners;//屋主姓名

                        that.registerNum =houseownerinfo.hjh;//户籍号
        ...
        methods: {
            initbuihouse: function(infos) {
                ...
                this.register = infos.rhdjsj; //入户登记时间
                this.huxing = infos.huXing; //户型
                this.carNum = infos.carNum; //车辆数
                this.carId = infos.carID; //车牌号
                // this.houseownerId = infos.idNumber; //屋主身份证
                // this.houseownerName = infos.homeOwners; //屋主姓名

            },
            registerf: function() {
                var that = this;
                if (!this.isBrowser) {
                    UICore.openTimeComponent2(this.register);
                    api.addEventListener({
                        name: 'buildingTime'
                    }, function(ret, err) {
                        if (ret) {
                            that.register = ret.value.key1;
                        } else {
                            alert(JSON.stringify(err));
                        }
                    });
            ...
            residentPropertyf:function(){
                console.log("户属性");
                  if (!this.isBrowser) {
                      var that = this;
                      var defaultVal = this.residentProperty; //获取默认值
                      if (defaultVal != null && defaultVal != "") {
                          this.residentProperty_arr.forEach(function(value, index, arr) {
                              if (value.text == that.residentProperty) {
                                  arr[index].status = "selected";
                              }
                          })
                      }
                      UICore.openSelect3(this.residentProperty_arr, this.residentProperty, "residentProperty");
                      api.addEventListener({
                          name: 'residentProperty'
                      }, function(ret, err) {
                          if (ret) {
                              that.residentProperty = ret.value.key1;
                              that.residentPropertyId = ret.value.key2 - 1;
                                console.log(that.residentProperty);
                          } else {
                              alert(JSON.stringify(err));
                          }
                      });
            ...
            submit: function() {
                if (this.isBrowser) {
                    ...
                    postjson.residentProperty=this.residentPropertyId
                    ...
                    postjson.rhdjsj = this.register; //入户登记时间
                    postjson.huXing = this.huxing; //户型
                    postjson.carNum = this.carNum; //车辆数
                    postjson.carID = this.carId; //车牌号
                    // postjson.idNumber = this.houseownerId; //屋主身份证
                    // postjson.homeOwners = this.houseownerName; //屋主姓名
                    postjson.hjh = this.registerNum; //户籍号
