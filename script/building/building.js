...
apiready = function() {
    new Vue({
        ...
        data: {
            ...
            constructionApplicationsId: "",
            architecturalNature: "", //建筑性质
            architecturalNatureId: "",
            buildingtypes: "", //建筑类型
            buildingtypesId: "",
            buildingStorey: "", //建筑层数
            buildingHeight: "", //建筑高度
        ...
        created: function() {
            ...
            jsonData.data.forEach(function(value) {
                ...
                if (value.parentKey == "BD_Basic_MaintenanceFund") { //维修基金
                    that.fund_arr.push({
                        text: value.extendAttributeValue,
                        status: 'normal',
                        key: value.extendAttributeKey
                ...
                if (value.parentKey == "BD_Basic_HouseProperty") { //房产证
                    that.property_arr.push({
                        text: value.extendAttributeValue,
                        status: 'normal',
                        key: value.extendAttributeKey
                    });
                }
                if (value.parentKey == "BD_Basic_OwnerShip") { //产权归属
                    that.ownership_arr.push({
                        text: value.extendAttributeValue,
                        status: 'normal',
                        key: value.extendAttributeKey
                    });
                }
                if (value.parentKey == "BD_Basic_ManagementNew") { //物业管理
                    that.manager_arr.push({
                        text: value.extendAttributeValue,
                        status: 'normal',
                        key: value.extendAttributeKey
                    })
                ...
                api.ajax({
                    ...
                    if (ret) {
                        if (ret.success) {
                            ...
                            that.neighbourhood = ret.data.residentialName; //小区名称
                            ...
                            var index = parseInt(ret.data.management);
                            if (!isNaN(index) && index != 0) {
                                that.manager_arr[index - 1].status = "selected";
                                that.manager = that.manager_arr[index - 1].text;
                            } //物业管理
                            that.cellId = ret.data.residentialId; //关联小区id

                            that.fund_arr.forEach(function(val) {
                                if (val.key == that.params.maintenanceFund) {
                                    val.status = "selected";
                                    that.fund = val.text;
                                }
                            }); //维修基金
                            that.property_arr.forEach(function(val) {
                                if (val.key == that.params.houseProperty) {
                                    val.status = "selected";
                                    that.property = val.text;
                                }
                            }); //房产证

                            that.ownership_arr.forEach(function(val) {
                                if (val.key == that.params.ownerShip) {
                                    val.status = "selected";
                                    that.ownership = val.text;
                                }
                            }); //产权归属
        ...
        methods: {
            cellqueryf: function() { //小区名称
                console.log("小区名称");
                var that = this;
                api.openWin({
                    name: 'courtyardqueryforbui',
                    url: '../courtyard/courtyardquery.html',
                    pageParam: {
                        from: 'building'
                    }
                });
                api.addEventListener({
                    ...
                    if (ret) {
                        ...
                        that.neighbourhood = ret.value.key1.name;
                        that.cellId = ret.value.key1.id;
                    } else {
                        alert(JSON.stringify(err));
                    }
                });
                ...
                if (!this.isBrowser) {
                    ...
                    api.addEventListener({
                        ...
                        if (ret) {
                            ...
                            that.constructionApplicationsId = ret.value.key2;
                        } else {
                            alert(JSON.stringify(err));
                        }
                    });
                ...
                if (!this.isBrowser) {
                    ...
                    api.addEventListener({
                        ...
                        if (ret) {
                            ...
                            that.architecturalNatureId = ret.value.key2;
                        } else {
                            alert(JSON.stringify(err));
                        }
                    });
                ...
                if (!this.isBrowser) {
                    ...
                    api.addEventListener({
                        ...
                        if (ret) {
                            ...
                            that.buildingtypesId = ret.value.key2;
                        } else {
                            alert(JSON.stringify(err));
                        }
                    });
            ...
            fundf: function() { //维修基金
                var that = this;
                if (!this.isBrowser) {
                    var defaultVal = this.fund; //获取默认值
                    if (defaultVal != null && defaultVal != "") {
                        this.fund_arr.forEach(function(value, index, arr) {
                            if (value.text == that.fund) {
                                arr[index].status = "selected";
                            }
                        })
                    }
                    UICore.openSelect3(this.fund_arr, this.fund, "fund");
                    api.addEventListener({
                        name: 'fund'
                    }, function(ret, err) {
                        if (ret) {
                            that.fund = ret.value.key1;
                            that.fundId = ret.value.key2;
                        } else {
                            alert(JSON.stringify(err));
                        }
                    });
            ...
            propertyf: function() { //房产证
                var that = this;
                if (!this.isBrowser) {
                    var defaultVal = this.property; //获取默认值
                    if (defaultVal != null && defaultVal != "") {
                        this.property_arr.forEach(function(value, index, arr) {
                            if (value.text == that.property) {
                                arr[index].status = "selected";
                            }
                        })
                    }
                    UICore.openSelect3(this.property_arr, this.property, "property");
                    api.addEventListener({
                        name: 'property'
                    }, function(ret, err) {
                        if (ret) {
                            that.property = ret.value.key1;
                            that.propertyId = ret.value.key2;
                            console.log(JSON.stringify(ret));
                        } else {
                            alert(JSON.stringify(err));
                        }
            ...
            ownershipf: function() { //产权归
                var that = this;
                if (!this.isBrowser) {
                    var defaultVal = this.ownership; //获取默认值
                    if (defaultVal != null && defaultVal != "") {
                        this.ownership_arr.forEach(function(value, index, arr) {
                            if (value.text == that.ownership) {
                                arr[index].status = "selected";
                            }
                        })
                    }
                    UICore.openSelect3(this.ownership_arr, this.ownership, "ownership");
                    api.addEventListener({
                        name: 'ownership'
                    }, function(ret, err) {
                        if (ret) {
                            that.ownership = ret.value.key1;
                            that.ownershipId = ret.value.key2;
                        } else {
                            alert(JSON.stringify(err));
                        }
                    });
            ...
            managerf: function() {
                var that = this;
                if (!this.isBrowser) {
                    var defaultVal = this.manager; //获取默认值
                    if (defaultVal != null && defaultVal != "") {
                        this.manager_arr.forEach(function(value, index, arr) {
                            if (value.text == that.manager) {
                                arr[index].status = "selected";
                            }
                        })
                    }
                    UICore.openSelect3(this.manager_arr, this.manager, "manager");
                    api.addEventListener({
                        name: 'manager'
                    }, function(ret, err) {
                        if (ret) {
                            that.manager = ret.value.key1;
                            that.managerId = ret.value.key2;
                        } else {
                            alert(JSON.stringify(err));
                        }
                    });
            ...
            submit: function() {
                ...
                if (this.isBrowser) {
                    ...
                    postjson.MANAGEMENT = this.managerId; // 物业管理
                    postjson.CONTACTS = this.principal; // 采集人
                    postjson.CONTACT_INFO = this.contactNumber; // 联系电话
                    postjson.residentialName = this.neighbourhood; //小区名称
                    postjson.residentialId = this.cellId; //关联小区id

                    postjson.maintenanceFund = this.fundId; //维修基金
                    postjson.houseProperty = this.propertyId; //房产证
                    postjson.ownerShip = this.ownershipId; //产权归属
                    postjson.photo = "";
